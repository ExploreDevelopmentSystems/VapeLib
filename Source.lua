...
local function RXCN_fake_script() -- Toggle.LocalScript 
	local script = Instance.new('LocalScript', Toggle)

	local button = script.Parent -- Reference to the TextButton
	local frame = button:WaitForChild("Frame") -- Reference to the child Frame
	local TweenService = game:GetService("TweenService") -- Reference to TweenService
	
	local toggle = false -- State to keep track of the toggle
	
	-- Swapped Positions
	local falsePos = UDim2.new(0.467, 0, 0, 0) -- {0.467, 0}, {0, 0}
	local truePos = UDim2.new(0, 0, 0, 0) -- {0, 0}, {0, 0}
	
	-- Faster Tween Information
	local tweenInfo = TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out) -- 0.25 seconds duration, with easing style
	
	-- Function to toggle the frame's position with tween
...
local function RKDOEU_fake_script() -- Asset.Setup 
